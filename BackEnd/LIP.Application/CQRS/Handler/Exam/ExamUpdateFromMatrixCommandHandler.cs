...
{
    public class ExamUpdateFromMatrixCommandHandler : IRequestHandler<ExamUpdateFromMatrixCommand, ExamUpdateFromMatrixResponse>
    {
        ...
        public ExamUpdateFromMatrixCommandHandler(IExamRepository examRepository, IExamMatrixRepository examMatrixRepository, IPracticequestionRepository practicequestionRepository)
        {
            _examRepository = examRepository;
            _examMatrixRepository = examMatrixRepository;
            _practicequestionRepository = practicequestionRepository;
        }
        public async Task<ExamUpdateFromMatrixResponse> Handle(ExamUpdateFromMatrixCommand request, CancellationToken cancellationToken)
        {
            ...
            foreach (var detail in matrix.Exammatrixdetails)
            {
                ...
                if (matched.Count() != detail.Quantity)
                {
                    ...
                    {
                        IsSuccess = false,
                        Message = $"Matrix detail mismatch for {detail.LessonName}. Expect {detail.Quantity}, got {matched.Count}."
                    };
                }
            }

            var invalid = selectedQuestions
                .Where(q => !matrix.Exammatrixdetails.Any(md => md.QuestionType == q.QuestionType && md.Difficulty == q.Difficulty))
                .ToList();
