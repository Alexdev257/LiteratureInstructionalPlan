...
namespace LIP.Application.CQRS.Handler.Exam
{
    public class GetAllExamQueryHandler : IRequestHandler<GetAllExamQuery, GetAllExamResponse>
    {
        ...
        {
            var exams = await _examRepository.GetAllAsync( new ExamGetAllQuery { GradeLevelId = request.GradeLevelId, ExamTypeId = request.ExamTypeId, CreatedBy = request.CreatedBy, IsAdmin = request.IsAdmin });
