...
public class
    ExamCreateManualFromMatrixCommandHandler : IRequestHandler<ExamCreateManualFromMatrixCommand,
    ExamCreateManualFromMatrixResponse>
{
    ...
    public ExamCreateManualFromMatrixCommandHandler(IExamRepository examRepository,
    ...
    public async Task<ExamCreateManualFromMatrixResponse> Handle(ExamCreateManualFromMatrixCommand request,
        ...
        foreach (var detail in matrix.Exammatrixdetails)
        {
            ...
            if (matchedQuestions.Count() != detail.Quantity)
                return new ExamCreateManualFromMatrixResponse
                {
                    IsSuccess = false,
                    Message =
                        $"Invalid number of questions for '{detail.LessonName}' ({detail.QuestionType}-{detail.Difficulty}). Expected {detail.Quantity}, got {matchedQuestions.Count()}."
                };
        }
