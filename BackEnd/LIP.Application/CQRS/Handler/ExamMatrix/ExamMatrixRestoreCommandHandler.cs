...
namespace LIP.Application.CQRS.Handler.ExamMatrix
{
    public class ExamMatrixRestoreCommandHandler : IRequestHandler<ExamMatrixRestoreCommand, ExamMatrixRestoreResponse>
    {
        ...
        public async Task<ExamMatrixRestoreResponse> Handle(ExamMatrixRestoreCommand request, CancellationToken cancellationToken)
        {
            var check = await _examMatrixRepository.GetAsync(new ExamMatrixGetQuery { MatrixId = request.MatrixId, IsAdmin = true });
