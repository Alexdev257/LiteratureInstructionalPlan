...
{
    public class PracticequestionUpdateCommandHandler : IRequestHandler<PracticequestionUpdateCommand, PracticeQuestionUpdateResponse>
    {
        ...
        public PracticequestionUpdateCommandHandler(IPracticequestionRepository practicequestionRepository, IUserRepository userRepository, IGradelevelRepository gradelevelRepository)
        {
        ...
        public async Task<PracticeQuestionUpdateResponse> Handle(PracticequestionUpdateCommand request, CancellationToken cancellationToken)
        {
            var question = await _practicequestionRepository.GetAsync(new LIP.Application.CQRS.Query.Practicequestion.PracticequestionGetQuery { QuestionId = request.QuestionId });
            if (question == null)
            {
            ...
            var gradelevel = await _gradelevelRepository.GetAsync(new GradelevelGetQuery { GradeLevelId = request.GradeLevelId.Value });
            if (gradelevel == null)
            {
                return new PracticeQuestionUpdateResponse
                {
