...
export default function UpdateQuestionForm({ initialData }: UpdateQuestionFormProps) {
  const { user } = useSessionStore();
  const { usePutQuestion } = useQuestion();
  ...
  useEffect(() => {
    const loadedAnswers = initialData.answer || [];
    const loadedCorrect = (initialData.correctAnswer || []).map(a => a.label);

    setAnswers(loadedAnswers.length > 0 ? loadedAnswers : [{ label: "A", text: "" }, { label: "B", text: "" }]);
    setCorrectAnswerLabels(loadedCorrect);

    if (initialData.questionType === "3" && initialData.correctAnswer?.[0]?.text) {
      setEssayAnswer(initialData.correctAnswer[0].text);
    }
  }, [initialData, form]);
  ...
  const onSubmit = (data: QuestionInput) => {
    usePutQuestion.mutate({
      id: initialData.questionId,
      data
    }, {
      onSuccess: (res) => {
        if (res.isSuccess) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <div className="flex justify-end gap-3">
          <Button
            ...
            disabled={usePutQuestion.isPending}
          >
            <ArrowLeft className="w-4 h-4 mr-2" />
            Quay lại
          </Button>
          <Button type="submit" disabled={usePutQuestion.isPending}>
            {usePutQuestion.isPending ? (
              <>
